...
Implementation status: Issue #9 establishes Journal-level suspended and terminal Run semantics, including audit-only late results after cancellation. Issue #10 implements the per-run lease, heartbeat, expiry, independent cancellation request, and propagation to active operations required for cross-process enforcement.
