...
这是一个以学习 Agent 工程为目的的本地 TypeScript 项目。当前只完成 Issue #2 的 traceable planning slice：
...
5. Run 持久停在 `waiting_plan_approval`，等待后续 ticket 的精确版本审批。
...
记下输出中的 `runId`，然后可以在任意后续进程读取同一 Runtime Home：

```bash
node dist/src/cli.js inspect --runtime-home .runtime --run-id <run-id>
...
- `src/application/research-agent-runtime.ts`：command-oriented 主 seam，以及外部模型调用不占用 SQLite 事务的原因。
...
- `tests/runtime/research-agent-runtime.test.ts`：真实临时 SQLite 与跨 runtime 重开的公共行为测试。
...
- 当前 CLI 使用通用的 `ScriptedModel` 计划，仅用于验证端到端 planning slice，不代表 live model 能力。
- Source Scope 在当前 ticket 中被验证并冻结记录；realpath containment 和真实源读取属于后续 ticket。
